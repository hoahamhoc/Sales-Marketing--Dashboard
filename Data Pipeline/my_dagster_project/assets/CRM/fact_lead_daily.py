...
SALES_TEAM_ID = "xxxx"  # Sales team ID
...
def fact_lead_daily(context) -> None:
    ...
    # CONFIG
    # =========================
    API_URL         = "https://xxxx/v1/crm/leads/report"  # CRM API endpoint
    BEARER_TOKEN    = token
    N_DAYS          = 5
    MAX_WORKERS     = 10
    TIMEOUT_SECONDS = 60
    ...
    context.log.info(f"Fetch {N_DAYS} days: {TARGET_DATES[-1]} to {TARGET_DATES[0]}")

    # =========================
    # STEP 1: GET SOURCE_IDS FROM API
    # =========================
    context.log.info("Step 1: Fetching source_ids from API...")
    ...
    if not SOURCE_IDS:
        raise Exception("No source_ids found in this date range.")
    ...
    # =========================
    context.log.info("Step 2: Fetching data in parallel...")
    ...
    context.log.info(f"Total requests: {len(tasks)} ({len(TARGET_DATES)} days x {len(SOURCE_IDS)} sources)")
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for future in as_completed(future_to_task):
            ...
            if result.get("success"):
                ...
                context.log.warning(f"Failed: {result['date']} | {result['source_id'][:12]} | {result.get('error')}")

    context.log.info(f"Completed {completed} tasks | Failed: {len(failed_requests)}")
    ...
    context.log.info(f"Total: {len(df):,} rows | Sources: {df['source_id'].nunique()} | Reps: {df['sales_rep_id'].nunique()}")

    # =========================
    # STEP 4: LOAD TO SQL SERVER
    # =========================
    context.log.info("Step 4: Loading to SQL Server...")
    ...
    with engine.begin() as conn:
        for target_date in TARGET_DATES:
            ...
            """), {"d": target_date})
            context.log.info(f"Deleted {result.rowcount} rows for {target_date}")
    ...
    context.log.info(f"Inserted {len(df):,} rows into [{SCHEMA_NAME}].[{TABLE_NAME}]")
    context.log.info(f"Total leads   : {df['totalLead'].sum():,}")
    context.log.info(f"Total quantity: {df['totalQuantity'].sum():,.0f}")

    if failed_requests:
        context.log.warning(f"{len(failed_requests)} failed requests:")
