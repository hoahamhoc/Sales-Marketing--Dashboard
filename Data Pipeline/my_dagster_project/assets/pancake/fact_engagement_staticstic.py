...
def fact_engagement_staticstic(context):
    ...
    # CONFIG
    # =========================
    BASE_URL     = "https://xxxx/api/v1/statistics/customer_engagements"  
    ACCESS_TOKEN = token
    ...
    def fetch_engagement_by_date(page_id, date, retry_count=0):
        ...
        try:
            ...
            if response.status_code == 429:
                ...
                context.log.warning(f"Rate limit [{page_id}|{date}]. Waiting {retry_after}s...")
    ...
    context.log.info(f"Completed {completed} tasks")
    ...
    context.log.info(f"Total: {len(df_final)} rows")
    ...
    context.log.info(f"Inserted {len(df_final)} rows into [{SCHEMA_NAME}].[{TABLE_NAME}]")
    context.log.info(f"Total pages: {df_final['page_id'].nunique()}")