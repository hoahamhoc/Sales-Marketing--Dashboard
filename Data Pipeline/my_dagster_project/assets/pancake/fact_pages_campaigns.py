...
def fact_pages_campaigns(context):
    ...
    context.log.info(f"Date range: {START_DATE.strftime('%Y-%m-%d')} to {END_DATE.strftime('%Y-%m-%d')}")

    # =========================
    # STEP 1: GET PAGES FROM SQL
    # =========================
    context.log.info("Step 1: Fetching page list from SQL Server...")
    ...
    context.log.info(f"Fetched {len(PAGE_IDS)} pages")
    ...
    # =========================
    # STEP 3: API FETCH FUNCTION
    # =========================
    def fetch_with_retry(page_id, date_range, retry_count=0):
        session = get_session()
        url     = f"https://xxxx/api/v1/pages/{page_id}/statistics/pages_campaigns"  # Pancake API endpoint
        params  = {
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                context.log.warning(f"Rate limit [{page_id}]. Waiting {wait}s...")
                time.sleep(wait)
    ...
    # =========================
    context.log.info("Step 4: Fetching data in parallel...")
    ...
    context.log.info(f"Completed {completed} tasks | Failed: {len(failed_tasks)}")

    # =========================
    # STEP 4b: RETRY FAILED TASKS
    # =========================
    if failed_tasks:
        context.log.info(f"Retrying {len(failed_tasks)} failed tasks (waiting 30s)...")
        time.sleep(30)
        ...
        for page_id, date in failed_tasks:
            ...
            if status == "success":
                ...
                context.log.warning(f"Retry failed [{page_id}|{date_str}] -> placeholder (error)")
    ...
    # STEP 5: PROCESS DATA
    # =========================
    context.log.info("Step 5: Processing data...")
    ...
    context.log.info(f"Total: {len(df)} rows | Columns: {df.columns.tolist()}")

    # =========================
    # STEP 6: LOAD TO SQL SERVER
    # =========================
    context.log.info("Step 6: Loading to SQL Server...")
    ...
    with engine.begin() as conn:
        ...
        """), {"d": delete_date})
        context.log.info(f"Deleted data from {delete_date}")
    ...
    if "spend" in df.columns:
        context.log.info(f"Total spend: {df['spend'].sum():,.0f}")